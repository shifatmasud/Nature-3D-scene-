...
const createFlowerTexture = () => {
  // FIX: Increased texture resolution for sharper, less pixelated flowers.
  const size = 128;
...
export const createFlowers = (
    ...
    try {
        ...
        const totalFlowers = patchCount * flowersPerPatch;
        ...
        // --- 1. STEMS ---
        // FIX: Taper stem to a point to prevent the top cap from clipping through the flower billboard.
        const stemGeo = new THREE.CylinderGeometry(0.0, 0.015, 1, 3);
        stemGeo.translate(0, 0.5, 0); // Pivot at the bottom
        ...
        stemMaterial.onBeforeCompile = (shader) => {
            ...
            shader.vertexShader = shader.vertexShader.replace(
                ...
                // This creates a bend, where sway is proportional to height (position.y)
        ...
        const stemsMesh = new THREE.InstancedMesh(stemGeo, stemMaterial, totalFlowers);
        ...
        headMaterial.onBeforeCompile = (shader) => {
            ...
            shader.vertexShader = shader.vertexShader.replace(
                ...
                #include <common>
                uniform float uTime;
                attribute vec3 aFlowerInfo; // x: headSize, y: stemHeight, z: id
                float hash(vec2 p) { return fract(sin(dot(p, vec2(12.9898, 78.233))) * 43758.5453); }
                `
            );
            ...
            shader.vertexShader = shader.vertexShader.replace(
                '#include <begin_vertex>',
                `
                // --- Spherical Billboard in World Space ---
                ...
                // FIX: Increased y-offset to ensure flower sits visually lower on the stem, hiding the tip.
                float yOffset = -0.4;
                ...
                // --- Convert back to local space for the pipeline ---
        ...
        const headsMesh = new THREE.InstancedMesh(headGeo, headMaterial, totalFlowers);
        ...
        for (let i = 0; i < patchCount; i++) {
            ...
            for(let j=0; j<flowersPerPatch; j++) {
                ...
                stemsMesh.setMatrixAt(instanceIdx, stemDummy.matrix);
                
                // HEAD
                // FIX: Removed z-fighting epsilon, as the cone tip removes the surface-on-surface issue.
                headDummy.position.set(flowerX, patchY + stemHeight, flowerZ);
                headDummy.updateMatrix();
                headsMesh.setMatrixAt(instanceIdx, headDummy.matrix);

                const headColor = palette[Math.floor(Math.random() * palette.length)];
                headsMesh.setColorAt(instanceIdx, headColor);

                // Pass info to shader
                flowerInfo[instanceIdx * 3 + 0] = headSize;
                flowerInfo[instanceIdx * 3 + 1] = stemHeight;
                flowerInfo[instanceIdx * 3 + 2] = Math.random() * 100; 
        ...
        cleanup = () => {
            scene.remove(stemsMesh);
            scene.remove(headsMesh);
            stemGeo.dispose();
            headGeo.dispose();
